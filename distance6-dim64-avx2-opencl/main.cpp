...
const cl_int vector_dims = 64;
//cl_int vector_nums = 16;
//cl_int vector_nums = 1024;
const cl_int vector_nums = 8096;
const cl_int vector_size = vector_dims * vector_nums;

const cl_int colA = sqrt(vector_dims);
const cl_int rowA = sqrt(vector_dims);
const cl_int colB = sqrt(vector_dims);
...
template <typename T>
void checkResults(T* a, T* b, size_t rowA, size_t colA, size_t colB) {
...
template <typename T>
void checkDistance(T* a, T* b, size_t dim, size_t number) {
	std::cout << "\ncheckDistance number:" << number << std::endl;
...
#ifdef DEBUG
		std::cout << "checkDistance omp:\t" << omp_get_thread_num() << std::endl;
#endif
		//if(j % width == 0)
		//	std::cout << std::endl;
		T tmp = 0.0f;
...
float fvec_L2sqr_avx2(const float * x,
                 const float * y,
                 size_t d)
{
...
void opencl_distance(int deviceid) {
	cout << "opencl_distance deviceid:\t" << deviceid << endl;
	...
	cl_platform_id platform = platforms[deviceid];
...
void avx2_distance() {
	...
	struct timeval end;
	float sum = 0.0f;
	std::cout << "avx2_distance vector_nums:" << vector_nums << std::endl;

	gettimeofday(&start, NULL);
	for(size_t j=0; j<vector_nums; ++j) {
		//if(j % width == 0)
		//	std::cout << std::endl;
		sum = fvec_L2sqr_avx2(ha, hb, vector_dims);
		//std::cout << sum << " ";
	}
	//std::cout << std::endl;

	gettimeofday(&end, NULL);
	std::cout << "AVX2 Kernel time:\t" << ((end.tv_sec - start.tv_sec)*1e6 + (end.tv_usec - start.tv_usec)) << " us" << std::endl;
}
...
int main(int argc, char** argv) {
	if(argc > 1) {
		...
		// 0,1: OpenCL CPU/GPU, 2,3: reference/avx2
		if(2 == testcase)
			reference_distance();
		else if(3 == testcase)
			avx2_distance();
		else
			opencl_distance(testcase);
	}
	return 0;
}
